...
def add_dim_supplier_surrogate_key(df: DataFrame) -> DataFrame:
    order_by_list = ["supplier_id_source", "supplier_name"]
    cleaned_df = add_surrogate_key(
        df=df, surrogate_key="SK_dim_supplier", ordered_list=order_by_list)
    return cleaned_df


def reorder_dim_supplier_columns(df: DataFrame) -> DataFrame:
    ordered_list = ["SK_dim_supplier", "supplier_id_source",  "supplier_code", "supplier_name",
