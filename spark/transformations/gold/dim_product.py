...
def add_dim_product_surrogate_key(df: DataFrame) -> DataFrame:
    order_by_list = ["product_id_source", "product_name"]
    cleaned_df = add_surrogate_key(
        df=df, surrogate_key="SK_dim_product", ordered_list=order_by_list)
    return cleaned_df


def reorder_dim_product_columns(df: DataFrame) -> DataFrame:
    ordered_list = ["SK_dim_product", "product_id_source", "sku", "product_name",  "category",
