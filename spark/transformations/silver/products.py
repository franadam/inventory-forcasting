...
from spark.common.dataframe_utils import read_csv
...
def clean_unit_price_eur(df: DataFrame) -> DataFrame:
    cleaned_df = df.withColumn("unit_price_eur", F.regexp_replace(
...
def clean_unit_weight_kg(df: DataFrame) -> DataFrame:
    cleaned_df = df.withColumn("unit_weight_kg", F.regexp_replace(
        F.col("unit_weight_kg"), "-", ""))
